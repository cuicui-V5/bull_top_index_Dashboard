...
# 历史顶点（你已认可）
BULL_TOPS = [ "2015-06-12","2018-01-24", "2021-02-18", "2024-10-08", "2025-8-25"]
BULL_TOPS = [pd.to_datetime(d) for d in BULL_TOPS]

# 回测参数
ADVANCE_DAYS = 10   # 要求信号在顶部前5天内出现
SIGNAL_THRESHOLD = 93.0  # 触发阈值（可调整）
...
# ------------------ 网格生成（粗网格、灵敏导向） ------------------
def generate_weight_grid(step=0.1, has_search=True):
    """
    我们先在五大类上做粗网格:
    ...
    """
    grid = []
    choices = [round(i * step, 10) for i in range(int(1 / step) + 1)]
    if has_search:
        choices_search = [x for x in choices if x <= 0.5]
    else:
        choices_search = [0.0]
    for w_liq in choices:
        for w_search in choices_search:
...
def main(argv=None):
    ...
    parser.add_argument("--step", type=float, default=0.1, help="grid step for coarse search (default 0.2). smaller -> more combos")
    ...
    has_search = "search_heat_z" in features.columns and not features["search_heat_z"].isna().all()

    # prepare grid
    print("生成网格（step=", args.step, ")...")
    grid = generate_weight_grid(step=args.step, has_search=has_search)
    print("网格大小：", len(grid))
