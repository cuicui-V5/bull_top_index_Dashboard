...
def load_hs300(path):
    ...
    if "日期" not in df.columns: raise ValueError("沪深300数据缺少'日期'列")
...
def load_csiall(path):
    df = try_read_csv(path)
    if "日期" not in df.columns: raise ValueError("中证全指数据缺少'日期'列")
    df["日期"] = parse_date_series(df["日期"])
...
def load_margin(path):
    df = try_read_csv(path)
    if "日期" not in df.columns: raise ValueError("融资融券数据缺少'日期'列")
    df["日期"] = parse_date_series(df["日期"])
...
def load_douyin(path):
    df = try_read_csv(path)
    if "日期" not in df.columns: raise ValueError("抖音搜索数据缺少'日期'列")
    search_col = next((c for c in df.columns if str(c).strip().replace(" ", "") in ["搜索量", "搜索指数", "Search", "search", "index"]), None)
...
def load_pe_data(path):
    """加载沪深300历史市盈率数据"""
    df = try_read_csv(path)
    if "日期" not in df.columns: raise ValueError("市盈率数据缺少'日期'列")
    if "滚动市盈率" not in df.columns: raise ValueError("市盈率数据缺少'滚动市盈率'列")
    df["日期"] = parse_date_series(df["日期"])
...
def build_features(merged):
    ...
    out["amplitude_heat_z"] = robust_rolling_z(out["amplitude_mean"], window=60, trend_window=None)
    ...
    out["margin_heat_z"] = robust_rolling_z(out["margin_total_log"], window=120, trend_window=long_trend) # 杠杆变化慢，窗口稍长
    ...
    out["price_accel_z"] = robust_rolling_z(out["ret_10d"], window=60, trend_window=None) # 窗口调整为60日
    ...
    out["pe_valuation_z"] = robust_rolling_z(out["hs300_pe_ttm"], window=120, trend_window=long_trend)
...
def main(argv=None):
    ...
    try:
        ...
        merged = merged.sort_values("日期").set_index("日期")
        merged["hs300_pe_ttm"] = merged["hs300_pe_ttm"].ffill() # 估值数据也进行前向填充
        merged = merged.reset_index()
    except Exception as e:
    ...
    cols = ["日期","hs300_close","hs300_pe_ttm","hs300_ret","hs300_turnover_log","hs300_amplitude","hs300_turnover_rate",
            ...
            "pe_valuation_z", # 新增
            "ma_spread","ma_spread_z","up_ratio","up_ratio_z",
            "crowding_z","escape_index_raw","escape_index_0_100","escape_signal","escape_level"]
